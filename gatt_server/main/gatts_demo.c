...
#define GATTS_SERVICE_UUID_TEST_A   0xFFFE
#define GATTS_CHAR_UUID_TEST_A      0xFFF2
#define GATTS_NUM_HANDLE_TEST_A     4


#define TEST_DEVICE_NAME            "DTH11"
...
void dht_notify_loop(void *param) {
    float temperature, humidity;
    char notify_data[40];

    while (1) {
        if (dht_read_float_data(DHT_TYPE_DHT11, GPIO_NUM_17, &humidity, &temperature) == ESP_OK) {
            snprintf(notify_data, sizeof(notify_data), "Temp: %.1f, Hum: %.1f", temperature, humidity);

            // Notificar solo si hay conexión
            if (gl_profile_tab[PROFILE_A_APP_ID].conn_id != 0xFFFF) {
                ...
                ESP_LOGI("DHT_NOTIFY", "🔁 Notificado: %s", notify_data);
            } else {
                ESP_LOGW("DHT_NOTIFY", "⚠️ Cliente no conectado.");
            }
        } else {
            ESP_LOGE("DHT_NOTIFY", "❌ Fallo al leer DHT11");
        }
...
void app_main(void)
{
	...
	xTaskCreate(dht_notify_loop, "dht_notify_loop", 4096, NULL, 5, NULL);
