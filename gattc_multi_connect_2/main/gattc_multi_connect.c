...
#define SERVICE_UUID_GYRO_MOUSE     0xFFF0
#define CHAR_UUID_GYRO_MOUSE        0xFFF1

#define SERVICE_UUID_DTH11        0xFFFE
#define CHAR_UUID_DTH11           0xFFF2

#define SERVICE_UUID_HEART_RATE     0x180D
#define CHAR_UUID_HEART_RATE        0x2A37

static esp_bt_uuid_t service_uuid_profile[3] = {
    {.len = ESP_UUID_LEN_16, .uuid = {.uuid16 = SERVICE_UUID_GYRO_MOUSE}},   // A
    {.len = ESP_UUID_LEN_16, .uuid = {.uuid16 = SERVICE_UUID_DTH11}},      // B
    {.len = ESP_UUID_LEN_16, .uuid = {.uuid16 = SERVICE_UUID_HEART_RATE}}    // C
};

static esp_bt_uuid_t char_uuid_profile[3] = {
    {.len = ESP_UUID_LEN_16, .uuid = {.uuid16 = CHAR_UUID_GYRO_MOUSE}},      // A
    {.len = ESP_UUID_LEN_16, .uuid = {.uuid16 = CHAR_UUID_DTH11}},         // B
    {.len = ESP_UUID_LEN_16, .uuid = {.uuid16 = CHAR_UUID_HEART_RATE}}       // C
};
...
static const char remote_device_name[3][20] = {"Gyro Mouse", "DTH11", "Xiaomi Pad 6"};
...
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CFG_MTU_EVT:
        ...
        esp_ble_gattc_search_service(gattc_if, param->cfg_mtu.conn_id, &service_uuid_profile[0]);
        break;
    case ESP_GATTC_SEARCH_RES_EVT: {
        ESP_LOGI(GATTC_TAG, "SEARCH RES: conn_id = %x is primary service %d", p_data->search_res.conn_id, p_data->search_res.is_primary);
        ESP_LOGI(GATTC_TAG, "start handle %d end handle %d current handle value %d", p_data->search_res.start_handle, p_data->search_res.end_handle, p_data->search_res.srvc_id.inst_id);
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == service_uuid_profile[0].uuid.uuid16) {
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGI(GATTC_TAG, "📥 Notificación recibida (%d bytes)", p_data->notify.value_len);

		char gyro_data[32] = {0};
		
		// Asegúrate de que no se desborda el buffer y que es null-terminated
		int len = p_data->notify.value_len < sizeof(gyro_data) - 1 ? p_data->notify.value_len : sizeof(gyro_data) - 1;
		memcpy(gyro_data, p_data->notify.value, len);
		gyro_data[len] = '\0';
		int gx = 0, gy = 0, gz = 0;
		if (sscanf(gyro_data, "%d,%d,%d", &gx, &gy, &gz) == 3) {
		    ESP_LOGI(GATTC_TAG, "🧭 Coordenadas Giroscopio: GX=%d, GY=%d, GZ=%d", gx, gy, gz);
		} else {
		    ESP_LOGW(GATTC_TAG, "❌ Formato de datos incorrecto: '%s'", gyro_data);
		}
        break;
...
static void gattc_profile_b_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CFG_MTU_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "Status %d, MTU %d, conn_id %d", param->cfg_mtu.status, param->cfg_mtu.mtu, param->cfg_mtu.conn_id);
        esp_ble_gattc_search_service(gattc_if, param->cfg_mtu.conn_id, &service_uuid_profile[1]);
        break;
    case ESP_GATTC_SEARCH_RES_EVT: {
        ESP_LOGI(GATTC_TAG, "SEARCH RES: conn_id = %x is primary service %d", p_data->search_res.conn_id, p_data->search_res.is_primary);
        ESP_LOGI(GATTC_TAG, "start handle %d end handle %d current handle value %d", p_data->search_res.start_handle, p_data->search_res.end_handle, p_data->search_res.srvc_id.inst_id);
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == service_uuid_profile[1].uuid.uuid16) {
            ESP_LOGI(GATTC_TAG, "UUID16: %x", p_data->search_res.srvc_id.uuid.uuid.uuid16);
            get_service_b = true;
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGI(GATTC_TAG, "🌡️ Notificación recibida desde DHT11 (%d bytes)", p_data->notify.value_len);
        char dht_data[40] = {0};
	    int len = p_data->notify.value_len < sizeof(dht_data) - 1 ? p_data->notify.value_len : sizeof(dht_data) - 1;
	    memcpy(dht_data, p_data->notify.value, len);
	    dht_data[len] = '\0';
	    ESP_LOGI(GATTC_TAG, "📦 Datos recibidos: '%s'", dht_data);
	
	    float temp = 0.0f, hum = 0.0f;
	
	    // Extrae temperatura y humedad del string "Temp: 25.3, Hum: 58.0"
	    if (sscanf(dht_data, "Temp: %f, Hum: %f", &temp, &hum) == 2) {
	        ESP_LOGI(GATTC_TAG, "🌡️ Temperatura: %.1f °C, 💧 Humedad: %.1f %%", temp, hum);
	    } else {
	        ESP_LOGW(GATTC_TAG, "❌ Formato no válido: '%s'", dht_data);
	    }
...
static void gattc_profile_c_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CFG_MTU_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "Status %d, MTU %d, conn_id %d", param->cfg_mtu.status, param->cfg_mtu.mtu, param->cfg_mtu.conn_id);
        esp_ble_gattc_search_service(gattc_if, param->cfg_mtu.conn_id, &service_uuid_profile[2]);
        break;
    case ESP_GATTC_SEARCH_RES_EVT: {
        ESP_LOGI(GATTC_TAG, "SEARCH RES: conn_id = %x is primary service %d", p_data->search_res.conn_id, p_data->search_res.is_primary);
        ESP_LOGI(GATTC_TAG, "start handle %d end handle %d current handle value %d", p_data->search_res.start_handle, p_data->search_res.end_handle, p_data->search_res.srvc_id.inst_id);
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == service_uuid_profile[2].uuid.uuid16) {
            ESP_LOGI(GATTC_TAG, "UUID16: %x", p_data->search_res.srvc_id.uuid.uuid.uuid16);
            get_service_c = true;
    ...
    case ESP_GATTC_NOTIFY_EVT:
        //ESP_LOGI(GATTC_TAG, "Notificacion recibida, valor en hexadecimal ");
        //ESP_LOG_BUFFER_HEX(GATTC_TAG, p_data->notify.value, p_data->notify.value_len);
        if (p_data->notify.value_len > 1) {
        uint8_t flags = p_data->notify.value[0];
        uint16_t bpm = 0;

        if (flags & 0x01) {
            // Heart Rate in 16-bit format
            if (p_data->notify.value_len >= 3) {
                bpm = p_data->notify.value[1] | (p_data->notify.value[2] << 8);
            }
        } else {
            // Heart Rate in 8-bit format
            bpm = p_data->notify.value[1];
        }

        ESP_LOGI(GATTC_TAG, "❤️ Frecuencia cardíaca: %d bpm", bpm);
   		} else {
        	ESP_LOGW(GATTC_TAG, "Notificación demasiado corta para extraer BPM");
    	}
        break;
    case ESP_GATTC_WRITE_DESCR_EVT:
        if (p_data->write.status != ESP_GATT_OK){
